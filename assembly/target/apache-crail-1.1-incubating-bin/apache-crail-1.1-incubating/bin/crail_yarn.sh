...
function printUsage {
  echo "Usage: crail-yarn.sh <numWorkers> <pathHdfs> [masterAddress]"
...
CRAIL_HOME="$(cd "${SCRIPT_DIR}/.."; pwd)"

source "${SCRIPT_DIR}/common.sh"

NUM_WORKERS=$1
HDFS_PATH=$2
MASTER_ADDRESS=${3:-${CRAIL_MASTER_HOSTNAME}}

CRAIL_TARFILE="crail.tar.gz"
rm -rf $CRAIL_TARFILE
tar -C $CRAIL_HOME -zcf $CRAIL_TARFILE \
  ./alluxio-application-master.sh \
  bin conf lib libexec jars

JAR_LOCAL=${CRAIL_HOME}/yarn/crail-yarn.jar

echo "Uploading files to HDFS to distribute alluxio runtime"

${HADOOP_HOME}/bin/hadoop fs -mkdir -p ${HDFS_PATH}
${HADOOP_HOME}/bin/hadoop fs -put -f ${CRAIL_TARFILE} ${HDFS_PATH}/${CRAIL_TARFILE}
${HADOOP_HOME}/bin/hadoop fs -put -f ${JAR_LOCAL} ${HDFS_PATH}/crail.jar
${HADOOP_HOME}/bin/hadoop fs -put -f ${SCRIPT_DIR}/alluxio-yarn-setup.sh ${HDFS_PATH}/alluxio-yarn-setup.sh
${HADOOP_HOME}/bin/hadoop fs -put -f ${SCRIPT_DIR}/crail-application-master.sh ${HDFS_PATH}/alluxio-application-master.sh
...
${YARN_HOME}/bin/yarn jar ${JAR_LOCAL} org.apache.crail.yarn.Client \
